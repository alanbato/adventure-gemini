...
def _phase_equip_and_clear_snake(world, state):
    ...
    _run(world, state, [
        ...
        "get rod",
        "e", "get cage",
        "w", "w", "w",       # → bird chamber r13
        "drop rod", "get bird", "get rod",
        "w", "d", "d",       # → hall of mt king r19
...
def _phase_diamonds_and_pirate(world, state):
    ...
    _run(world, state, [
        "u", "w",             # → east bank fissure r17
        "wave rod", "drop rod",
        "w", "get diamond",   # → west bank r27
    ])
...
def _phase_pyramid_and_emerald(world, state):
    ...
    _run(world, state, [
        "get keys", "get food", "get bottle", "get water",
        "plugh", "plover",    # → plover room r100
        "ne", "get pyramid",  # → dark room r101
        "s", "plover", "plugh", "drop pyramid",
    ])
    _assert_at(state, 3)
    ...
    _run(world, state, [
        "drop keys", "drop lamp", "drop food", "drop bottle",
    ])
    ...
    state.current_room = 99  # alcove
    _run(world, state, [
        "get keys", "get lamp", "get food", "get bottle",
    ])
...
def _phase_plant_eggs_trident(world, state):
    ...
    _run(world, state, [
        "plugh", "s", "d", "w", "d", "w", "w",  # → swiss cheese r66
        "w", "w",             # → east twopit r67 → west twopit r23
        "d", "pour water",    # west pit r25, 1st watering
        ...
        "d", "pour water",    # west pit r25, 2nd watering
        "u", "e",             # → east end twopit r67
        "d", "get oil", "u",  # east pit r24
        "w", "d",             # → west pit r25
        "climb",              # → r26 → narrow corridor r88
        "w",                  # → giant room r92
        "get eggs",
        "n", "pour oil", "drop bottle",  # → door room r94 (north of giant)
        "n", "get trident",   # → waterfall cavern r95
    ])
...
def _phase_troll_bear_spices(world, state):
    ...
    _run(world, state, [
        "w", "d",             # → steep incline r91 → large low room r72
        "sw", "u",            # → SW side of chasm (troll!)
        "throw eggs",
        "cross",              # → NE side of chasm
        "ne", "e", "se", "s", "e",  # → barren room (bear!)
        "feed bear", "open chain",
        "drop keys", "get chain", "get bear",
        "w", "w", "n",       # → fork in path
        "ne", "e",            # → boulder chamber
        "get spices",
        "w", "s", "w", "w",  # → NE side of chasm
...
def _phase_fee_fie_foe_foo(world, state):
    ...
    _run(world, state, [
        ...
        "w", "w",             # → east twopit r67 → west twopit r23
        "d",                  # → west pit r25
        "climb", "w",         # → narrow corridor → giant room r92
        "fee", "fie", "foe", "foo",
        "get eggs",
        "s", "d", "u",       # narrow corr → west pit → west twopit
    ])
...
def test_full_350_walkthrough(world: World) -> None:
    ...
    Since our engine doesn't have dwarf/pirate AI, we manipulate state
    for those encounters and use real commands for everything else.
    """
    state = new_game_state(world)
